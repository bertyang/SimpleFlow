...
function GetFlowId() {
    ...
    return Flow.getElementsByTagName("BaseProperties").item(0).getAttribute("flowId");
...
function objFocusedOff(){   
   ...
   if(FlowXML.value!=''){
	   ...
	   eval('document.getElementById("' + FOCUSEDOBJID + '").StrokeColor="' + focusedOffColor + '"');
	   FOCUSEDOBJID = '';
	   FOCUSEDOBJTYPE = '';
   }
}
...
function stepContextMenu(stepId,stepType){
    //菜单源文本
  var menuSrc = '<menu><entity id="c0"><description>Edit Step</description><image>inc/contextMenu/images/edit_obj.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();editStep("'+stepId+'","'+stepType+'");</onClick><contents></contents></entity>';
  if(stepType!='BeginStep' && stepType!='EndStep') menuSrc+= '<entity id="c1"><description>Delete Step</description><image>inc/contextMenu/images/del_obj.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();delStep("'+stepId+'");</onClick><contents></contents></entity>';
  menuSrc += '<entity id="c2"><description>Refresh</description><image>inc/contextMenu/images/refresh_vml.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();redrawVML();</onClick><contents></contents></entity></menu>';
...
function actionContextMenu(actionId){
  //菜单源文本
  var menuSrc = '<menu><entity id="c0"><description>Edit Action</description><image>inc/contextMenu/images/edit_obj.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();editAction("'+actionId+'");</onClick><contents></contents></entity>';
  menuSrc+= '<entity id="c1"><description>Delete Action</description><image>inc/contextMenu/images/del_obj.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();delAction("'+actionId+'");</onClick><contents></contents></entity>';
  menuSrc+= '<entity id="c2"><description>Refresh</description><image>inc/contextMenu/images/refresh_vml.gif</image><fontcolor>'+MenuTextColor_enable+'</fontcolor><onClick>cleancontextMenu();redrawVML();</onClick><contents></contents></entity></menu>';
  showContextMenu(menuSrc);
}
...
function workFlowDialog(id,type){
	...
	switch (type){
	    ...
	    case 'Action': dialogURL = id == null ? '_actiondialog.html' : '_actiondialog.html?actionid=' + id + "&formId=" + GetFlowId(); break;
		case 'Flow': dialogURL = id==null?'_flowdialog.html':'_flowdialog.html?flowid='+id;break;		
	} 
	var dialog = window.showModalDialog(dialogURL, window, "dialogWidth:573px; dialogHeight:460px; center:yes; help:no; resizable:no; status:no") ;
	drawTreeView();
}
